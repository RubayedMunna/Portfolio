    ...
    const renderItems = (items, sectionIcon) => (
        <div className="grid grid-cols-1 gap-6 mt-6">
            {items.map((item, index) => (
                <div
                    key={index}
                    className={`w-4/5 mx-auto relative transition-transform duration-300 transform hover:scale-105 ${
        ...
        <div className="flex items-center space-x-4">
            <div className="w-12 h-12 rounded-full bg-white/20 backdrop-blur-md flex items-center justify-center shadow-md border border-white/30">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="white" className="w-6 h-6">
                    {/* Graduation cap icon for academic services */}
                    <path strokeLinecap="round" strokeLinejoin="round" d="M12 14l9-5-9-5-9 5 9 5z" />
                    <path strokeLinecap="round" strokeLinejoin="round" d="M12 14v7" />
                    <path strokeLinecap="round" strokeLinejoin="round" d="M12 21l4-4m-8 0 4 4" />
                </svg>
            </div>
            <div>
                <h1 className="text-4xl font-extrabold tracking-tight">Academic Services</h1>
